...
import { annualSavingsPercent, type BillingPeriod, PRICING_TIERS } from '@/lib/pricing';

const MAX_ANNUAL_SAVINGS = Math.max(...PRICING_TIERS.map((t) => annualSavingsPercent(t)));
            ...
            {MAX_ANNUAL_SAVINGS > 0 ? (
              <span
                className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] font-semibold transition-colors ${
                  period === 'annual' ? 'bg-white/25 text-white' : 'bg-accent/15 text-accent'
                }`}
              >
        ...
        {PRICING_TIERS.map((tier) => {
          ...
          return (
            ...
            >
              ...
              <p className="mb-5 min-h-[2.5rem] text-sm text-muted-foreground">{tier.tagline}</p>

              <div className="mb-1 flex items-baseline gap-1">
                <span className="text-4xl font-bold tracking-tight text-foreground">${amount}</span>
                {!isFree ? <span className="text-sm text-muted-foreground">{unit}</span> : null}
              </div>
