  ...
  const cards = [
    {
      ...
      sub:
        stats.burnedPct != null
          ? `${pct(stats.burnedPct)} of initial supply`
    ...
    },
    {
      ...
      sub:
        stats.lockedPct != null ? `${pct(stats.lockedPct)} of supply` : 'Liquidity & vesting locks',
    },
    {
      icon: Wallet,
      label: 'Dev / treasury',
    ...
    },
    {
      ...
      value:
        stats.holders != null
          ? `${stats.holdersCapped ? '' : ''}${stats.holders.toLocaleString('en-US')}`
          : '—',
      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto mt-4">
            Supply, holders, burns, locks and team holdings for {TOKEN_TICKER}, read live from
            Solana.
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3 mb-4">
          <HeadlineStat
            ...
            sub={stats.totalSupply != null ? `of ${compact(stats.totalSupply)} total` : undefined}
          />
          <HeadlineStat label="Price" value={usdPrice(stats.priceUsd)} sub="via Jupiter" />
          <HeadlineStat
            label="Market cap"
            value={usdBig(stats.marketCapUsd)}
            sub="price × supply"
          />
        </div>
        ...
        {SHOW_CREATOR_REWARDS && stats.creatorRewardsSol != null && (
          <div className="mt-4 rounded-2xl border border-accent/30 bg-gradient-to-b from-accent/[0.08] to-transparent p-8 text-center">
            ...
            <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-muted-foreground">
              Lifetime {TOKEN_TICKER} trading fees — the funding that pays for full-time work on
              Voicebox.{' '}
              <a
          ...
          <div className="mt-4 rounded-xl border border-border bg-card/40 backdrop-blur-sm p-6">
            ...
            <ul className="space-y-3">
              {stats.lockedBreakdown.map((l) => (
                <li key={l.account} className="flex items-center gap-3 text-sm">
          ...
          <div className="mt-4 rounded-xl border border-border bg-card/40 backdrop-blur-sm p-6">
            ...
            <ul className="divide-y divide-border/60">
              {stats.topHolders.map((h, i) => (
                <li key={h.owner} className="flex items-center gap-3 py-2.5 text-sm">
                  ...
                  <span className="ml-auto tabular-nums text-foreground">{compact(h.amount)}</span>
